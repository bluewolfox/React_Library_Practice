...
export const Tree: React.FC = (): JSX.Element => {
  ...
  const Contents: React.FC = () => {
    const TreeItems = (arg: { children: TypeTree[] }): any => {
      return arg.children.map((item) => {
        ...
        const onClickExpanded = () => {
          if (!ableChild) return;
          setTreeData(findExpandTargetID(treeData));
        };
    ...
    return <div className="tree-item-wrapper">{TreeItems({ children: treeData })}</div>;
  };
